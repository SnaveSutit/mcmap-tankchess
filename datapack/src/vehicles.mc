...
LOOP(config.tc.Vehicle.all, v) {
	function summon_<%v.name%> {
		...
		scoreboard players set .cmd v <%v.customModelData%>
		scoreboard players operation .cmd v += .activeSkin.cmdOffset v

		execute align xyz positioned ~.5 ~ ~.5 as @e[type=area_effect_cloud,tag=new,distance=..0.1] run {
		...
		execute as @e[type=armor_stand,tag=new,distance=..1] run {
			scoreboard players operation @s V.id = #last_id v
			execute store result entity @s ArmorItems[-1].tag.CustomModelData int 1 run scoreboard players get .cmd v
