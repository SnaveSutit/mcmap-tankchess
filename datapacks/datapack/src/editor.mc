...
function clear_loaded_map {
	execute as @e[tag=obstacle,x=990,y=11,z=-10, dx=21,dy=10,dz=21] at @s run function obstacles:remove_obstacle
}
...
function tick {
	...
	execute as @a[tag=editor] at @s anchored eyes positioned ^ ^ ^ run {
		...
		# Detect when the player switches items
		data modify storage util:nbt a set value 'none'
		data modify storage util:nbt a set from entity @s SelectedItem.tag.customItemId
		function util:check_nbt_equality
		execute if score #return v matches 0 run {
			title @a actionbar ["Switched item to ", {"storage": "util:nbt", "nbt": "a"}]
			function editor:kill_ghost
			data modify storage func:input 
		}
		# Store old item
		data modify storage util:nbt b set from storage util:nbt a
			...
			scoreboard players set #distance v 250
			{
				execute unless block ~ ~ ~ #minecraft:air run {
					...
					execute unless entity 0-0-0-0-0 run {
						# say summon
						execute align xyz positioned ~.5 ~1 ~.5 run summon marker ~ ~ ~ {Tags:['editor.raycast_marker'],UUID:[I;0,0,0,0]}
					}
		...
		execute if predicate editor:holding/remove_piece run {
			execute at 0-0-0-0-0 run {
				...
				execute if score #return v matches 1 run {
					...
					execute as @e[tag=collided_entity,limit=1] run {
						function util:glow_stack {color: 16711680}
		...
		# 		kill @e[tag=editor.ghost,tag=!ghost_<%v.name%>]
		...
		# 				execute as @e[type=item_display,tag=aj.rig_root,distance=..0.01,limit=1] run {
		# 					tag @s add ghost_<%v.name%>
		# 					tag @s add editor.ghost
		# 					function util:glow_stack {color: 0}
		...
		# 					setblock ~ 5 ~ minecraft:piston[facing=up]
		...
		# 		kill @e[tag=editor.ghost,tag=!ghost_<%i%>]
		# 		execute as 0-0-0-0-0 at @s rotated ~ 0 run {
...
function remove_piece {
	...
	execute if entity @s[tag=vehicle] run {
		function util:kill_stack
...
function set_editor_tools_vehicles {
	...
	item replace entity @s container.1 with editor:place_vehicle_mv 1
